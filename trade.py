...
class Trade:
    ...
    def render_global_assets(self, screen, game_font):
        ...
        for k, v in self.global_assets.items():
            row = [k, self.global_assets[k]["magnitude"], self.global_assets[k]["price"]]
    ...
    def create_possible_bids(self, settlements):
        ...
        for s in settlements:
            if s.preferred_good != "":
                ...
                affordable_magnitude = s.settlement_goods.calculate_affordable_magnitude(preferred_good_price)
                if affordable_magnitude > 0:
                    self.ladder.create_ladder_entry(s, s.preferred_good, affordable_magnitude, preferred_good_price, EntryType.BID)
            ...
            for s in ss:
                ...
                for trading_good in self.possible_trading_goods:
                    ...
                    if s.settlement_goods.is_in_stock(trading_good):
                        self.ladder.create_ladder_entry(s, trading_good, s.trading_goods[trading_good], trading_good_price, EntryType.ASK)
